...
def api_list_states(request):
    ...
    lst = []
    for state in states:
        lst.append({"name": state.name, "abbreviation": state.abbreviation})
    # sorted_list = sorted(lst, key=lambda d: d[state.name])
    return JsonResponse({
        "states": lst
    })

@require_http_methods(["GET"])
def api_list_conference_location(request):
